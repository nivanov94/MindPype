# -*- coding: utf-8 -*-
"""
Created on Tue Jun 23 22:42:05 2020

@author: Nick
"""


from data_ext import ParticipantClassifierExtractor
...
def extract_metric_hyperparams(cfg):
    ...
    hyperparams = ['channels','freq_bands','classes',
...
def run_participant(participant,cfg):
    ...
    outfile = (cfg['output_file'] + "{}-ws{}-ss{}-wt{}"
               + '-' + timestr + '.csv')
    
    i = 1
    total_sets = len(metric_hyperparams)
    for hyp_set in metric_hyperparams:
        print("\t|\Extracting classifier metrics with hyperparams:",hyp_set)
        ...
        participant_clsf_file = cfg['clsf_file_tmp'].format(participant)
        
        PME = ParticipantClassifierExtractor(participant_clsf_file,
                                         list(classes),
                                         list(channels),
                                         fbands,
                                         win_sz,
...
if __name__ == "__main__":
    config_file = "D:\BCI\BCI_Capture\data\MI_datasets\BCIComp_MDM_online_extracting_cfg.json"
